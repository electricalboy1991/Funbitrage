...
def calculate_total_position_duration_and_formula(positions):
    ...
    for position in positions:
        ...
        total_formula_value *= (1 + 0.01*20 * (-funding_avg*duration+(entry_value-exit_value)-0.1))
...
app.layout = html.Div(style={'height': '100vh', 'display': 'flex', 'flexDirection': 'column'}, children=[
    html.Div(style={'display': 'flex', 'justifyContent': 'space-between'}, children=[
        html.Div(children=[
            ...
            html.Span("복리 수익률 % Lev 20배*[-해당 기간 평균 펀비*일수+(진입-청산)-0.1(수수료*2, 슬리피지 등]", style={'font-size': '70%', 'margin-left': '10px', 'vertical-align': 'middle'})
        ]),
        html.H2(id='position-duration', style={'flex': '0 1 auto', 'margin': 'auto 0'}),
        html.H2(id='funding-avg', style={'flex': '0 1 auto', 'margin-right': '10px'})
    ]),
...
def perform_backtest(symbols, df, entry_threshold, exit_threshold):
    ...
    for symbol in symbols:
        ...
        for i in range(len(symbol_df)):
            ...
            if symbol_df.iloc[i]['괴리율'] > symbol_df.iloc[i]['진입 점선'] and symbol_df.iloc[i]['괴리율'] > 1:
                ...
                current_position_end = exit_date
                break
...
def update_graph(selected_symbols, exit_threshold, entry_threshold):
    ...
    total_funding_avg = sum(df_funding[(df_funding['date'] >= pos['entry_date']) & (df_funding['date'] <= pos['exit_date'])]['펀딩비'].mean() if not df_funding.empty else 0.03 for pos in positions) / len(positions)

    for position in positions:
        symbol_df = filtered_df[filtered_df['symbol'] == position['symbol']]
        data.extend([go.Scatter(x=position['entry_line']['x'], y=position['entry_line']['y'], mode='lines', name=f'{position["symbol"]} 진입 점선', line=dict(dash='dot', color='blue')),
                     go.Scatter(x=position['exit_line']['x'], y=position['exit_line']['y'], mode='lines', name=f'{position["symbol"]} 청산 점선', line=dict(dash='dot', color='red')),
                     go.Scatter(x=symbol_df['date'], y=symbol_df['괴리율'], mode='lines', name=f'{position["symbol"]} 괴리율', yaxis='y1'),
                     go.Scatter(x=[position['entry_date'], position['exit_date']], y=[position['entry_y'], position['exit_y']], fill='tozeroy', fillcolor='rgba(0, 0, 255, 0.2)', mode='none', name=f'{position["symbol"]} 포지션'),
